...
const PortfolioDetail = () => {
  ...
  // 현재 로그인한 사용자 ID (백엔드 쿠키 처리라 하더라도, 프론트엔드에서 비교를 위해 임시로 localStorage 사용)
  ...
  // 내 프로젝트 목록 가져오기 (내가 만든 프로젝트만)
  const fetchMyProjects = async () => {
    try {
      const data = await getMyProjects(); // 백엔드가 현재 사용자 쿠키 정보를 이용하여 내 프로젝트만 반환하거나, 전체 목록 후 필터링 가능
      setMyProjects(data);
    } catch (error) {
  ...
  // 초대 버튼 클릭 시 모달 열기 (자신의 포트폴리오는 초대할 수 없음)
  ...
  const handleInviteConfirm = async () => {
    ...
    try {
      setInviting(true);
      // alertApi.inviteToProject(선택한 프로젝트ID, 포트폴리오 소유자ID(초대 대상))
